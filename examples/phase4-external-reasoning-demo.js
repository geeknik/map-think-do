...
class Phase4Demo {
  ...
  async demoMathematicalReasoning() {
    ...
    const scenarios = [
      ...
      },
      {
        ...
        thought:
          'Perform statistical analysis on the data [23, 45, 67, 34, 56, 78, 89, 12, 34, 56]',
      },
    ];
  ...
  async demoCreativeReasoning() {
    ...
    const scenarios = [
      ...
      },
      {
        ...
        thought:
          'Combine the concepts of artificial intelligence and urban gardening to create something novel',
      },
      {
      ...
      },
      {
        ...
        thought:
          'Create metaphors for quantum computing that help explain it to non-technical people',
      },
    ];
    ...
    const scenarios = [
      {
        ...
        thought:
          'Calculate the optimal dimensions for a creative art installation that uses mathematical patterns like Fibonacci sequences',
      },
      {
        title: 'Analytical + Creative Synthesis',
        thought:
          'Analyze the mathematical beauty in nature and generate creative ideas for biomimetic designs',
      },
      {
        title: 'Complex Problem Solving',
        thought:
          'Design a creative solution for optimizing traffic flow that involves both mathematical modeling and innovative thinking',
      },
    ];
  ...
  async demoToolOrchestration() {
    ...
    console.log(
      `${colors.yellow}Demonstrating how the AGI orchestrates multiple external tools...${colors.reset}\n`
    );
    ...
    if (externalPlugin) {
      ...
      console.log(
        `  • Average Impact: ${((externalPlugin.average_impact_score || 0) * 100).toFixed(1)}%`
      );
      console.log(
        `  • Response Time: ${(externalPlugin.average_response_time || 0).toFixed(0)}ms\n`
      );
    }
  }
  ...
  async demoAdaptiveLearning() {
    ...
    console.log(
      `${colors.yellow}Demonstrating how the system learns and adapts tool usage...${colors.reset}\n`
    );
    ...
    for (let i = 0; i < learningScenarios.length; i++) {
      ...
      console.log(
        `  📈 Cognitive Efficiency: ${(cognitiveState.cognitive_efficiency * 100).toFixed(1)}%`
      );
      console.log(
        `  🎯 Pattern Recognition: ${cognitiveState.pattern_recognition_active ? 'Active' : 'Inactive'}`
      );
      console.log(
        `  💡 Insight Potential: ${(cognitiveState.insight_potential * 100).toFixed(1)}%\n`
      );
    }
  }
    ...
    try {
      ...
      if (result.interventions.length > 0) {
        ...
        result.interventions.forEach((intervention, index) => {
          console.log(
            `  ${index + 1}. ${colors.cyan}[${intervention.type}]${colors.reset} ${intervention.content}`
          );
      ...
      if (result.insights.length > 0) {
        ...
        result.insights.forEach((insight, index) => {
          console.log(
            `  ${index + 1}. ${colors.yellow}[${insight.type}]${colors.reset} ${insight.description}`
          );
      ...
      console.log(
        `  • Metacognitive Awareness: ${(state.metacognitive_awareness * 100).toFixed(1)}%`
      );
      ...
      console.log(
        `  • Breakthrough Likelihood: ${(state.breakthrough_likelihood * 100).toFixed(1)}%`
      );
      console.log();
    } catch (error) {
...
╚══════════════════════════════════════════════════════════════╝${colors.reset}\n`);
  ...
  printSummary() {
    ...
    console.log(
      `  • Metacognitive Awareness: ${(cognitiveState.metacognitive_awareness * 100).toFixed(1)}%`
    );
    console.log(`  • Creative Pressure: ${(cognitiveState.creative_pressure * 100).toFixed(1)}%`);
    console.log(
      `  • Breakthrough Likelihood: ${(cognitiveState.breakthrough_likelihood * 100).toFixed(1)}%`
    );
    console.log();
    ...
    console.log(
      `${colors.bright}${colors.green}✨ Phase 4 External Reasoning Capabilities Demonstrated:${colors.reset}`
    );
    ...
    console.log(
      `${colors.bright}${colors.cyan}🚀 The AGI system now has enhanced external reasoning capabilities!${colors.reset}`
    );
