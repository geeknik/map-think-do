...
async function testBasicErrorHandling() {
  ...
  const result = await boundary.execute(() => Promise.resolve('success'), {
    component: 'test',
    method: 'success',
  });
  ...
  try {
    await boundary.execute(() => Promise.reject(new Error('test error')), {
      component: 'test',
      method: 'error',
    });
...
async function testCircuitBreaker() {
  ...
  for (let i = 0; i < 2; i++) {
    try {
      await boundary.execute(() => Promise.reject(new Error('circuit test')), {
        component: 'test',
        method: 'circuit',
      });
    } catch (error) {
