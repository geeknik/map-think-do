...
export class RolePrimingPlugin extends BasePromptPlugin {
  ...
  constructor() {
    const metadata: PluginMetadata = {
      name: 'role-priming',
      description: 'Provides different cognitive personas and thinking styles for contextual reasoning',
      version: '1.0.0',
      author: 'map-think-do',
      capabilities: [
  ...
  private initializeRoles(): void {
    const roles: CognitiveRole[] = [
      {
        name: 'strategist',
        description: 'High-level strategic thinker focused on long-term planning and systems thinking',
        strengths: ['systems-thinking', 'long-term-planning', 'pattern-recognition', 'risk-assessment'],
      ...
      {
        name: 'engineer',
        description: 'Detail-oriented problem solver focused on implementation and technical feasibility',
        strengths: ['technical-analysis', 'implementation-focus', 'constraint-awareness', 'optimization'],
      ...
      {
        name: 'skeptic',
        description: 'Critical thinker focused on finding flaws, risks, and alternative explanations',
        strengths: ['critical-analysis', 'risk-identification', 'assumption-challenging', 'devil\'s-advocate'],
        domains: ['all'],
      ...
      },
      {
        ...
        strengths: ['data-analysis', 'evidence-evaluation', 'systematic-thinking', 'quantification'],
      ...
      {
        name: 'philosopher',
        description: 'Deep thinker focused on fundamental questions, ethics, and conceptual clarity',
        strengths: ['conceptual-clarity', 'ethical-reasoning', 'fundamental-questions', 'logical-rigor'],
      ...
      {
        name: 'pragmatist',
        description: 'Practical thinker focused on what works, resource constraints, and actionable solutions',
      ...
      {
        name: 'synthesizer',
        description: 'Integrative thinker focused on combining diverse perspectives and finding common ground',
  ...
  private detectSimilarThoughts(thoughts: Array<{ thought: string }>): boolean {
    ...
    const keyWords = thoughts.map(t => 
      t.thought.toLowerCase().split(/\s+/).filter(word => word.length > 4)
    );
  ...
  private recommendRole(context: PromptContext): CognitiveRole | null {
    ...
    for (const [roleName, role] of this.availableRoles) {
      ...
      // Historical success
      const roleSuccesses = this.roleHistory.filter(h => 
        h.role === roleName && h.success
      ).length;
  ...
  generateTemplates(context: PromptContext): Record<string, (args: Record<string, string>) => PromptResult> {
    ...
    templates['select-role'] = (args) => {
      ...
      const roleOptions = Array.from(this.availableRoles.values())
        .map(role => `**${role.name}**: ${role.description} (Strengths: ${role.strengths.join(', ')})`)
        .join('\n');
...
Now, fully embody this ${role.name} persona and use the code-reasoning tool to approach the problem from this perspective. Think, speak, and reason as a ${role.name} would.`
  ...
  private getRoleMindset(role: CognitiveRole): string {
    const mindsets: Record<string, string> = {
      strategist: "Think big picture. Consider long-term consequences. Look for patterns and systems. Ask 'What's the ultimate goal?' and 'How does this connect to everything else?'",
      engineer: "Focus on implementation details. Consider constraints and trade-offs. Ask 'How would this actually work?' and 'What could break?'",
      skeptic: "Question everything. Look for flaws and risks. Ask 'What's wrong with this?' and 'What are we missing?'",
      creative: "Think outside the box. Make unexpected connections. Ask 'What if we tried something completely different?' and 'How can we break the rules?'",
      analyst: "Demand evidence. Look for data and patterns. Ask 'What does the data say?' and 'How can we measure this?'",
      philosopher: "Dig deep into fundamentals. Consider ethics and meaning. Ask 'Why does this matter?' and 'What are the underlying principles?'",
      pragmatist: "Focus on what works. Consider resources and constraints. Ask 'What's the simplest solution?' and 'What can we actually do?'",
      synthesizer: "Look for connections and integration. Bridge different perspectives. Ask 'How do these ideas fit together?' and 'What's the unified view?'"
    };
  ...
  private getRoleGuidelines(role: CognitiveRole): string {
    ...
    const roleGuidelines = guidelines[role.name] || ["Apply this role's perspective to the problem"];
