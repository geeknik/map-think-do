...
export abstract class BasePromptPlugin {
  ...
  shouldActivate(context: PromptContext): boolean {
    ...
    if (context.complexity !== undefined) {
      if (conditions.complexityThresholds?.min && context.complexity < conditions.complexityThresholds.min) {
        return false;
      }
      if (conditions.complexityThresholds?.max && context.complexity > conditions.complexityThresholds.max) {
        return false;
      }
    }
   ...
   */
  abstract generateTemplates(context: PromptContext): Record<string, (args: Record<string, string>) => PromptResult>;
  ...
  provideFeedback(promptName: string, success: boolean, confidence: number, context: PromptContext): void {
    ...
    console.error(`Plugin ${this.metadata.name} received feedback: ${promptName} - success: ${success}, confidence: ${confidence}`);
  }
