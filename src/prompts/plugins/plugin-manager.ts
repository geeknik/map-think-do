...
export class PluginManager {
   ...
   */
  private adjustPriorityBasedOnHistory(pluginName: string, basePriority: number, context: PromptContext): number {
  ...
  generatePrompts(context: PromptContext): Prompt[] {
    ...
    for (const activation of activePlugins) {
      ...
      try {
        ...
        console.error(`Error generating prompts from plugin ${activation.plugin.getMetadata().name}:`, error);
      }
    }
  ...
  generateTemplates(context: PromptContext): Record<string, (args: Record<string, string>) => PromptResult> {
    ...
    for (const activation of activePlugins) {
      try {
        ...
        console.error(`Error generating templates from plugin ${activation.plugin.getMetadata().name}:`, error);
      }
    }
   ...
   */
  getActivationHistory(context: PromptContext): Array<{ plugin: string; priority: number; timestamp: Date }> {
    const activePlugins = this.selectActivePlugins(context);
