...
export class DependencyContainer {
  ...
  async resolve<T>(token: string | symbol): Promise<T> {
    ...
    if (this.resolutionStack.has(token)) {
      ...
      throw new Error(`Circular dependency detected: ${stackArray.join(' -> ')} -> ${String(token)}`);
    }
  ...
  private async resolveSingleton<T>(token: string | symbol, descriptor: ServiceDescriptor<T>): Promise<T> {
  ...
  private async resolveScoped<T>(token: string | symbol, descriptor: ServiceDescriptor<T>): Promise<T> {
  ...
  private async resolveTransient<T>(token: string | symbol, descriptor: ServiceDescriptor<T>): Promise<T> {
    this.resolutionStack.add(token);
    try {
