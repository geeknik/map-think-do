...
export abstract class CognitivePlugin extends EventEmitter {
  ...
  protected updateMetrics(
    ...
    const complexity_key = context.complexity < 3 ? 'low' : context.complexity < 7 ? 'medium' : 'high';
...
export class CognitivePluginManager extends EventEmitter {
  ...
  async orchestrate(context: CognitiveContext): Promise<PluginIntervention[]> {
    ...
    try {
      ...
      const activationResults = (await Promise.all(activationPromises))
        .filter((result): result is { plugin: CognitivePlugin; activation: PluginActivation } => 
          result !== null && result.activation.should_activate
        );
      
      // Step 2: Sort by priority and apply adaptive adjustments
      if (this.adaptivePriority) {
        activationResults.sort((a, b) => {
        ...
        activationResults.sort((a, b) => b.activation.priority - a.activation.priority);
      }
      
      // Step 3: Select plugins considering conflicts and resource constraints
      const selectedPlugins = this.selectNonConflictingPlugins(
        activationResults,
        context
      );
      ...
      const interventions = (await Promise.all(interventionPromises))
        .filter((intervention): intervention is PluginIntervention => intervention !== null);
  ...
  private calculateAdaptivePriority(
    ...
    const complexityKey = context.complexity < 3 ? 'low' : context.complexity < 7 ? 'medium' : 'high';
    ...
    return basePriority * successMultiplier * domainMultiplier * complexityMultiplier * efficiencyMultiplier;
  }
